...
class masterbrandController extends Controller
{
    ...
    {
        try {
            $rowCount = DB::table('masterbrand')->count();
            DB::table('masterbrand')->truncate();

            Log::info('All data in masterbrand table has been deleted.', ['row_count' => $rowCount]);
            ...
            Log::error('Failed to delete all data from masterbrand table.', [
                'error' => $e->getMessage(),
    ...
    public function destroy(Request $req, int $id)
    {
        ...
        try {
            $todo = masterbrand::findOrFail($id);

            masterbrand::where('id', $id)->update(['deleted_by' => $user_id]);
            $todo->delete();
